...
with open('fileTxt/22.txt', 'r') as file:
...
def convertToExcel10(textC):
    ...
    nltk.pos_tag(text)
    if os.path.exists('dataExcel/22.xlsx'):
        print("...")
        df = pd.read_excel('dataExcel/22.xlsx', sheet_name='Sheet1')
        new_df = pd.DataFrame(nltk.pos_tag(text), columns=['Word', 'POS'])
        df = pd.concat([df, new_df], ignore_index=True)
        df.to_excel('dataExcel/22.xlsx', sheet_name='Sheet1', index=True)
        print("Success")
    else:
        print("...")
        df = pd.DataFrame(nltk.pos_tag(text), columns=['Word', 'POS'])
        df.to_excel('dataExcel/22.xlsx', index=True)
        print("Success")
